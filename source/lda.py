...
from source.parallel_calls import par_e_step
import numpy as np
...
class LDA(object):
    ...
    def gibbs_iteration(self,n_samples = 1,verbose = True,burn = True):
        ...
        for sample in range(n_samples):
            if verbose:
                print("Sample {} of {} (Burn is {})".format(sample,n_samples,burn))#修改python2的print
            for doc in self.corpus:
                di = self.doc_index[doc]
                for word in self.corpus[doc]:
                    wi = self.word_index[word]
...
class VariationalLDA(object):
    def __init__(self,corpus=None,K = 20,eta=0.1,
        ...
        if not self.corpus == None:
            ...
            print("Object created with {} documents".format(self.n_docs))#修改python2的print
            self.n_words = len(self.word_index)
            self.make_doc_index()
            if self.normalise > -1:
                print("Normalising intensities")#修改python2的print
